...
//					title: "사이트 이용 안내",
					...
					close_call_back:function() {
						window.location.href = elandmall.kidiMarketUtil.https("/main/initMain.action");
					},
...
//					title: "장기간 비밀번호 미변경안내",
					...
					createContent: function(layer) {
						...
						div.load("/login/initLoginPopLayer.action", {layer_gubun:'LONGTIMEPW'},  function() {
							...
							div.find("#confirm").click(function(){
								elandmall.kidiMarketMypage.link.modifyMemberInfo(elandmall.oneclick.getLoginId() , "60");
							});
							
							layer.show();
							$("#LONGTIMEWDINFO_LAYER").find(".btn_close > span").attr("class" , "");
							
							elandmall.kidiMarketUtil.setCookie({ name:'membState', domain: elandmall.global.cookie_domain, value:'', path: "/" });
						});
					}
				});
...
//					title: "임시 비밀번호 로그인 안내",
					...
					createContent: function(layer) {
						...
						div.load("/login/initLoginPopLayer.action", {layer_gubun:'TEMPPW'},  function() {
							...
							div.find("#confirm").click(function(){
								elandmall.kidiMarketMypage.link.modifyMemberInfo(elandmall.oneclick.getLoginId() , "70");
							});
				...
				elandmall.layer.createLayer({
					...
					createContent: function(layer) {
						...
						div.load("/login/initLoginPopLayer.action", {layer_gubun:'BIRTH'},  function() {
							...
							div.find("#confirm").click(function(){
								elandmall.kidiMarketMypage.link.coupon();
							});
							
							layer.show();
							$("#BIRTHDAY_LAYER").find(".btn_close > span").attr("class" , "");
							if($.type(pin.coupon_param) != "undefined") {
								elandmall.kidiMarketUtil.setCookie({ name:'coupon_param', domain: elandmall.global.cookie_domain, value:pin.coupon_param, path: "/" });	
							}
						});
					}
				});
								...
								obj.find("#confirm").click(function(){
									...
									$.ajax({
										...
										success: function(data) {
											...
											elandmall.kidiMarketUtil.setCookie({ name: "is_staff_don_agree_alram", value: "Y", age:365*10, path: "/", domain : elandmall.global.cookie_domain });
					...
					createContent: function(layer) {
						...
						div.load("/login/initLoginPopLayer.action", {layer_gubun:'GIFT'},  function() {
							...
							obj.find("#btn_today").click(function(){
								elandmall.kidiMarketUtil.setCookie({ name: "is_staff_gift_noti", value: "Y", age:1, path: "/", domain : elandmall.global.cookie_domain });
...
//			}
	...
	elandmall.login = function(p) {
				...
				createContent: function(layer) {
					...
					div.load("/login/initLoginLayer.action",p,function(){
						...
						div.find("#join_btn, #join_btn2").click(function() {
							setTimeout(function(){
								elandmall.kidiMarketOneclick.fnMemJoin();
							}, 100)
				        });
						
						div.find("#find_id").click(function() {
							setTimeout(function(){
								elandmall.kidiMarketOneclick.fnMemIDFind();
							}, 100)
				        });
						
						div.find("#find_passwd").click(function() {
							setTimeout(function(){
								elandmall.kidiMarketOneclick.fnMemPassFind();
							}, 100)
				        });
						...
						layer.show();
						$(".keyb").hide();
						if(webSizeMobileYn() != "Y"){ //PC화면일 경우 로그인 팝업 위치 및 크기 제어							
							$("#layer_pop").attr("style","left: 25%; top: 10%; width : 50%; height : 80%; border: 1px solid #000; display:block;");
							if($("#layer_pop").height() < 554){
								$("#layer_pop").attr("style","left: 25%; top: 10%; width : 50%; border: 1px solid #000; display:block;");
							}
							...
							$("body").attr("style","");
							$("#auto_lgn_id").prop( "checked", false );
							$("#auto_lgn_id").parent().hide();
						}
		...
		//바닥로그인	
           ...
           window.location.href = elandmall.kidiMarketUtil.https("/login/initLogin.action");
		}
	};
