...
function scrollReest(){ // 옵션 레이어가 열릴 경우 스크롤 재정의
	//재설정
	fnGoodsOpt();
	OrderScorller();
	//스크롤 상단으로
	 OrderScorll.scrollTo(0,0);
}
	...
	if($("div").hasClass("head")){
		...
		if($("div").hasClass("header_bn")){
			...
			if($("div").hasClass("cts_tit")){
				...
				$('.contents').css('padding-top',_top+38);
			}
		}
		function hd_dn(){
...
function square_lineup(slector){
	...
	wrap.each(function(){
		...
		$(this).css({'height':+size+'px'});
		if(img_rto > 1){
			//img.css({'width':+size+'px','height':'auto'}) /*2018.09.20 이미지 리사이즈 잘못되는 부분 수정*/
			img.css({'width':'100%','height':'auto'})
			img.css({'top':'-'+ (img.height()-size)/2 + 'px'})
		}
		if(img_rto < 1){
			//img.css({'width':'auto','height':+size+'px'}) /*2018.09.20 이미지 리사이즈 잘못되는 부분 수정*/
