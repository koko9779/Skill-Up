	...
	fnsearch = {
		...
		**/				
		headerSearchForm_submit	: function () {
			...
			//검색어가 입력되지 않았을 시 
			if(searchText == ""){
				var searchurl = $('#searchPrdMo').attr('data-url');
				var areano = $('#searchPrdMo').attr('data-area_no');
				var rel_no = $('#searchPrdMo').attr('data-rel_no');
				var banner_kind_cd = $('#searchPrdMo').attr('data-banner_kind_cd');
				var conts_form_cd = $('#searchPrdMo').attr('data-conts_form_cd');
				var conts_divi_cd = $('#searchPrdMo').attr('data-conts_divi_cd');
				var conts_dist_no = $('#searchPrdMo').attr('data-conts_dist_no');
				var disp_type_cd = $('#searchPrdMo').attr('data-disp_type_cd');
				var rel_divi_cd = $('#searchPrdMo').attr('data-rel_divi_cd');
				var move_cont_no =  $('#searchPrdMo').attr('data-move_cont_no');
				var placeholder =  $('#searchPrdMo').attr('placeholder');
				
				//등록된 url이나 컨텐츠가 있을 경우 해당 url로 이동
				if( (move_cont_no != undefined && move_cont_no != null && move_cont_no != '') || (searchurl != undefined && searchurl != null && searchurl != '') ){ 
					elandmall.kidikidiDisp.clickEvent({
						openwinyn:'N'
						, tr_yn:'Y'
						, banner_kind_cd: banner_kind_cd
						, conts_form_cd: conts_form_cd
						, conts_divi_cd: conts_divi_cd
						, conts_dist_no: conts_dist_no
						, sale_area_no: areano 
						, rel_divi_cd: rel_divi_cd
						, rel_no: rel_no
						, move_cont_no: move_cont_no
						, url: searchurl
						, disp_type_cd: disp_type_cd
						, iframeyn: 'N'
					});
				}
				else{
					if( placeholder && placeholder != "" && placeholder != "검색어를 입력해 주세요" ){
						$("#searchPrdMo").val(placeholder);
						$("#searchPrd").val(placeholder);
		...
		**/	
		srchFilterBox: function(search_type){
			...
			var url = "/kidikidi/search.action";
			
			if(search_type == "ctgr" || search_type == "init" || search_type == "del" || search_type == "sort"){
				url = "/kidikidi/commonEsDispCtg.action";
			}else{
				url = "/kidikidi/search.action";
				type = "GET";
			}
			...
			$("#mainSearchForm :input").each(function(index){
				if($(this).attr('name') == "kd_best_yn" || $(this).attr('name') == "kd_search_brand_no_list"){
					parameters += $(this).attr('name') + "=" + encodeURIComponent($(this).val()) + '&';
				}
			});
			
			//console.log("srchFilterBox = " + parameters);
			
			//console.log("srchFilterBox url = " + url);
			
			document.location.hash = "#" + parameters;
			
			//필터 처리시 body 스크롤 멈춤 제거
			if($("body").hasClass("scrollfix")) $("body").removeClass("scrollfix");
...
//					
//					//하나더 목록 추가
//					//$("#searchGoodsResultInc ul").html($("#tempGoodsList ul").html());
//					
//					//페이지 인덱스 설정
...
//					elandmall.lazyload();
//					
//					$("#searchGoodsResultInc").css("visibility", "");
//					
//					
//					//전체 갯수 수정
//					if($(".search_resultNum >p >span").length){
//						$(".search_resultNum >p >span").html(g_total);
//					}
//					
//					//필터바 갯수 수정
//					if($("#pop_filter_goods_cnt").length){
//						$("#pop_filter_goods_cnt").html(g_total + "개 상품");
//					}
//					
//					//필터바 셋팅
//					if($(".filterBar_filterList")){
//						
//						//filterSlider.reInit();
//					}
//					
...
//						if($(".filterBar_filterList").find("a").length){
...
//						if(!(search_type == "init" || search_type == "searchInit" || search_type == "sort")){
...
//						if(scrlPos > 0){
//						    
//						    if($(document).height() - $(window).height() < scrlPos){
//								fnMoreListPage();
//							}else{
...
//			});
		...
		**/	
		searchCategory: function( ctgr_cd ){
			window.location.href = "/kidikidi/initKidikidiDispCtg.action?kd_disp_ctg_no="+ctgr_cd;
		},
		...
		**/
		...
		initDetailFilterBox: function(search_type){
			...
			if($(".filterBar_filterList")){
				$(".filterBtn").removeClass("filterBtn-on");
				$(".filterBar_filterList").empty();
			}
			
			if(cnt > 0){
				if(search_type != null && search_type != ""){
					//fnsearch.srchFilterBox(search_type); //초기화시 상품 갱신 하지 않게 수정
				}
			}

			//우리아이 최적화 데이터 초기화
			// if($(".layerPop_fieldSet-kids").is(':visible') && search_type != null && search_type != ""){
			// 	$(".fieldSet_input_area span:nth-child(2)").find("input").prop("checked", true);
			// }
		},
