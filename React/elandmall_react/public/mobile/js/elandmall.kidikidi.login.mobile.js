			   ...
			   if(pin.is_staff == 'staff'){
				   if(elandmall.global.chnl_cd == '40' && $.type(elandmall.global.app_version) != "undefined" && elandmall.app.elandApp(elandmall.global.app_version)){
					   ...
					   if(layerCnt == 0 && $.type(isStaffAlram) != "undefined" && (isStaffAlram == '' || isStaffAlram == 'N')){
						   ...
						   elandmall.kidikidiUtil.setCookie({ name: "is_staff_alram_del", value: "Y", age:365*10, path: "/", domain : elandmall.global.cookie_domain });
					   }
...
//					title: "사이트 이용 안내",
					...
					close_call_back:function() {
						window.location.href = elandmall.kidikidiUtil.https("/main/initMain.action");
					},
					createContent: function(layer) {
						...
						div.load("/login/initLoginPopLayer.action", {layer_gubun:'SITEADD'},  function() {
							...
							html += "<li><a href=\"javascript://\" id=\"cancel\" class=\"btn_afterChange btn_bg06 c01\">취소</a></li>" ;
...
//					title: "장기간 비밀번호 미변경안내",
					...
					createContent: function(layer) {
						...
						div.load("/login/initLoginPopLayer.action", {layer_gubun:'LONGTIMEPW'},  function() {
							...
							var html = "";
							html = "<ul class=\"btn_list02\">" ;
							html += "<li><a href=\"javascript://\" id=\"cancel\" class=\"btn_afterChange btn_bg06 c04\">90일이후에변경</a></li>" ;
							...
							div.find("#confirm").click(function(){
								elandmall.kidikidiMypage.link.modifyMemberInfo(elandmall.oneclick.getLoginId() , "60");
							});
							...
							$('.lg_pop').center();
							$(".botNav").addClass("botNav-on");
							elandmall.kidikidiUtil.setCookie({ name:'membState', domain: elandmall.global.cookie_domain, value:'', path: "/" });
						});
					}
				});
...
//					title: "임시 비밀번호 로그인 안내",
					...
					createContent: function(layer) {
						...
						div.load("/login/initLoginPopLayer.action", {layer_gubun:'TEMPPW'},  function() {
							...
							var html = "";
							html = "<ul class=\"btn_list02\">" ;
							html += "<li><a href=\"javascript://\" id=\"cancel\" class=\"btn_afterChange btn_bg06 c01\">다음에변경</a></li>" ;
							html += "<li><a href=\"javascript://\" id=\"confirm\" class=\"btn_bg06 c01\"><strong>지금변경</strong></a></li>"; 
							html += "</ul>";
							div.append(html);
							...
							div.find("#confirm").click(function(){
								elandmall.kidikidiMypage.link.modifyMemberInfo(elandmall.oneclick.getLoginId() , "70");
							});
				...
				elandmall.layer.createLayer({
					...
					createContent: function(layer) {
						...
						div.load("/login/initLoginPopLayer.action", {layer_gubun:'BIRTH'},  function() {
							...
							var html = "";
							html = "<ul class=\"btn_list02\">" ;
							html += "<li><a href=\"javascript://\" id=\"cancel\" class=\"btn_afterChange btn_bg06 c01\">나중에확인하기</a></li>" ;
							...
							div.find("#confirm").click(function(){
								elandmall.kidikidiMypage.link.coupon();
							});
							...
							if($.type(pin.coupon_param) != "undefined") {
								elandmall.kidikidiUtil.setCookie({ name:'coupon_param', domain: elandmall.global.cookie_domain, value:pin.coupon_param, path: "/" });	
							}
						});
					}
				});
								...
								obj.find("#confirm").click(function(){
									...
									$.ajax({
										...
										success: function(data) {
											...
											elandmall.kidikidiUtil.setCookie({ name: "is_staff_don_agree_alram", value: "Y", age:365*10, path: "/", domain : elandmall.global.cookie_domain });
					...
					createContent: function(layer) {
						...
						div.load("/login/initLoginPopLayer.action", {layer_gubun:'GIFT'},  function() {
							...
							obj.find("#btn_today").click(function(){
								elandmall.kidikidiUtil.setCookie({ name: "is_staff_gift_noti", value: "Y", age:1, path: "/", domain : elandmall.global.cookie_domain });
	...
	elandmall.login = function(p) {
		...
		if(elandmall.global.chnl_cd == '40' && $.type(elandmall.global.app_version) != "undefined" && elandmall.app.elandApp(elandmall.global.app_version)){
			...
			// APP에서 웹뷰 호출 ==> /app/initAppHeader.action?path_url=/login/initLogin.action?gnbwebview=Y 하면/fo_mobile/webapp/main/api/initAppHeader.jsp 페이지 노출 is_app true로 셋팅됨
				...
				location.href=elandmall.kidikidiUtil.newHttps("/app/initAppHeader.action?path_url=/login/initLogin.action&gnbwebview=Y");
				return false;
			}
		}
		...
		if(p.popup){
			var layer = elandmall.layer.createLayer({
				...
				close_call_back:function() {
					// recapchat 보이는 문제 수정
					if($('div.grecaptcha-badge').is(':visible')) {
				...
				createContent: function(layer) {
					...
					div.load("/login/initLoginLayer.action",p,function(){
						...
						// 마켓통합에서는 아래를 적용함
						div.find('div.lg_top p.txt_info').eq(0).html('이랜드 통합멤버십 회원 로그인');
						div.find('div.ad_bn').hide();
						div.find('div.lg_btm').eq(0).find('li').addClass('lg_btm-item');
						div.find('div.lg_btm').eq(1).hide();

						//키디 마켓에선 한글자판 사용하지 않음
						$(".lg_top .keyb").hide();
						...
						div.find("#join_btn, #join_btn2").click(function() {
							setTimeout(function(){
								elandmall.kidikidiOneclick.fnMemJoin();
							}, 100)
				        });
						
						div.find("#find_id").click(function() {
							setTimeout(function(){
								elandmall.kidikidiOneclick.fnMemIDFind();
							}, 100)
				        });
						
						div.find("#find_passwd").click(function() {
							setTimeout(function(){
								elandmall.kidikidiOneclick.fnMemPassFind();
							}, 100)
				        });
						
						//비회원로그인
                        div.find("#nomember_btn").click(function() {
                        	//top에서 fnNomemLogin 함수 호출 시 상품상세의 정보를 받아오지 못하는 현상이 있어 iframe 존재 여부로 분기처리
                        	if($("#mainIframe").length != 0){
                        		$("#mainIframe")[0].contentWindow.elandmall.loginProc.fnNomemLogin();
		...
		//바닥로그인	
           ...
           window.location.href = elandmall.kidikidiUtil.https("/login/initLogin.action");
		}
	};
