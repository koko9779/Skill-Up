;(function($) {
	...
	fnsearchresent = {
		callKwdCookie: function( _kwd ){
		    ...
		    var newCookieStr = "";
		    var kwd = "";
		    if (elandmall.kidikidiUtil.getCookie(resultNm) != null) {    // 쿠키 존재 할 경우
		        cookieStr = elandmall.kidikidiUtil.getCookie(resultNm);        
		    ...
		    elandmall.kidikidiUtil.setCookie({ name: resultNm, value: kwd, age: 365*10, secure: false, path: "/" });
		},
		delKwdLst: function( obj ){
			...
			if (elandmall.kidikidiUtil.getCookie(resultNm) != null) {
				cookie_str = elandmall.kidikidiUtil.getCookie(resultNm);
				...
				elandmall.kidikidiUtil.setCookie({ name: resultNm, value: rtn_str, age: 365*10, secure: false, path: "/" });
			}
		...
		drawSearchedKwd: function(){
		    ...
		    var rct_kwd = "";
		    if (elandmall.kidikidiUtil.getCookie(resultNm) != null) { 
		    	temp_val = elandmall.kidikidiUtil.getCookie(resultNm);
		    	if(temp_val != ""){
			        ...
			        if (temp_str.length > 0 ) {         	
		            	...
		            	for (var i = temp_str.length-1; i >= 0; i--) {
		            		...
		            		if(count < maxHeaderResentKwd){
				            	...
				            	mySearchKwdHtml += "<li class=\"searchWords_item\">";
				            	mySearchKwdHtml += "<div class=\"searchWords_txt\">";
			            		...
			            		mySearchKwdHtml += "</div>";
			            		mySearchKwdHtml += "<div class=\"searchWords_util\">";
			            		mySearchKwdHtml += "<span class=\"searchWords_date\">" + tempDate + "</span>";
				            	mySearchKwdHtml += "<button type=\"button\" class=\"btn btn_del2 searchWords_delBtn\" onclick=\"fnsearchresent.delKwdLst(this);\" data-kwd=\""+ rct_kwd + "\" ><span class=\"blind\">검색어 삭제</span></button>";
				            	mySearchKwdHtml += "</div>";
				            	mySearchKwdHtml += "</li>";
			            ...
			            mySearchKwdHtml += "<div class=\"searchWords_btnArea\">";
			            mySearchKwdHtml += "<a class=\"searchWords_deleteAll\" href=\"javascript:;\" onclick=\"fnsearchresent.delKwdLstAll();\">검색어 전체삭제</a>";
			            mySearchKwdHtml += "</div>";
			        }	           
		    	}
		    }
		    
		    $("#latelySearch").html(mySearchKwdHtml);
		    
		    //최근 검색어 존재하지 않으면 영역 가림
		    if(mySearchKwdHtml == ""){
		    	$("#latelySearchArea").hide();
		    }
		}
	}
})(jQuery);